...
def ICPM(G, source, terminal, target_node, p_i, num_samples):
    nodes = [node for node in G.nodes if node != target_node and node != source and node != terminal]   # 소스와 터미널 노드, 요소 i 제외
    ...
    for _ in range(num_samples):
        G_copy = G.copy()
        G_copy.remove_node(target_node) # 구성 요소 i를 파괴로 가정했으므로 i 또한 제거
    ...
    Pr_sys_failure = failure_count / num_samples
    
    return Pr_sys_failure / p_i

target_node = int(input("노드 i(2~5): "))
ICPM_val = ICPM(G, source, terminal, target_node, p_i, num_samples)
print(f"역 조건부 확률 척도 ICPM = {ICPM_val}")
