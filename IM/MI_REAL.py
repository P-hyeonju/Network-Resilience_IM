...
# 구성요소 i가 완전히 신뢰할 수 있는 요소로 대체되었을 때 네트워크 실패 확률

def Pr_i_reliable(G, source, terminal, reliable_node, p_i, num_samples):
    nodes = [node for node in G.nodes if node != reliable_node and node != source and node != terminal]    # 소스와 터미널 노드, 요소 i 제외
    
    # 초기 실패 횟수 0으로 설정
    failure_count = 0
    
    # 몬테카를로 시뮬레이션을 통해 파괴 노드 결정
...
def MI(G, source, terminal, target_node, p_i, num_samples):
    Pr_net_failure_reliable_i = Pr_i_reliable(G, source, terminal, target_node, p_i, num_samples)
    Pr_net_failure_failed_i = Pr_i_failed(G, source, terminal, target_node, p_i, num_samples)
    
    return Pr_net_failure_failed_i - Pr_net_failure_reliable_i

target_node = int(input("노드 i(2~5):"))
MI_val = MI(G, source, terminal, target_node, p_i, num_samples)
print(f"한계 중요도 척도 MI = {MI_val}")
